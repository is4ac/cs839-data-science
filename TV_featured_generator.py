...
Features = 'stage1_docs/Data/Features/' # Trang: what does this folder contain?

# Global feature names that will be shared between modules
LOCATION_FEATURES = ['documentID', 'start_index', 'end_index']
OTHER_FEATURES = ['frequency', 'prefixed', 'suffixed', 'otherEntity',
                  'near_capitalized', 'name_suffix', 'common_word',
                  'first_name', 'actor_name', 'legislator_name']
TITLES_DICT = {}
FIRST_NAMES_DICT = {}
ACTOR_NAMES_DICT = {}
...
#RAW_TRAIN_CSV = DATA + 'raw_train_data.csv'
#RAW_TEST_CSV = DATA + 'raw_test_data.csv'
    ...
    for index in range(0, len(words)):
        for num_words in range(1, 5):
            if index + num_words <= len(words): 
                ...
                # get class_label before remove tags get other feature
                class_label = findClassLabel(word_string, start_tag, end_tag)
                # remove markedup tags
                ws = removeTags(word_string, start_tag, end_tag)
                # from here on only work with un-markedup strings, so used untagged_words list instead of words
                ...
                # check for if word_string could be otherEntity
                otherEntity = 0
...
def prune_data(data):
    ...
    if num_of_labels(reduced_set) != positive_count:
        print("Error: A label did not make it through! Check file {} for potential errors.".format(filename))
    return reduced_set 

def split_string(word_string):
    '''
    Split string into words and punctuations except for special cases:
        + string with tags
        + string starts and ends with alphabetic characters but
        contains hyphen. Eg. Ayman al-Zawahri
        + title words.
    ...
    specials = ['U.S.', 'C.I.A.', 'S.H.I.E.L.D'] # can extend this list
...
def generate_legislator_names():
    names = {}
    with open(LEGISLATORS_CURRENT_CSV, 'r', encoding = 'utf-8') as file:
    ...
    with open(LEGISLATORS_HISTORICAL_CSV, 'r', encoding = 'utf-8') as file:
        rd = csv.reader(file, delimiter=",", quotechar='"')
        for row in rd:
            if len(row) > 0:
                names[row[0]] = 1
...
def generate_actor_names():
    names = {}
    with open(ACTOR_NAMES1_CSV, 'r', encoding = 'utf-8') as file:
        for line in file:
            name_parts = line.split()
            for name in name_parts:
                names[name] = 1

    with open(ACTOR_NAMES2_CSV, 'r', encoding = 'utf-8') as file:
        for line in file:
            name_parts = line.split()
            for name in name_parts:
                names[name] = 1
...
def generate_first_names():
    names = {}
    with open(FIRST_NAMES_CSV, 'r', encoding = 'utf-8') as file:
        for line in file:
            words = line.split()
            if len(words) > 0:
...
def extractAndCreateCSV(file_names, csv_file):
    ...
    for filename in file_names:
        ...
        if os.path.isfile(MarkedUp + filename):
            ...
            reduced_data = prune_data(data)
            raw_df = pd.DataFrame(data, columns = headers)
            df = pd.DataFrame(reduced_data, columns = headers)
            fileID = fileID + 1
